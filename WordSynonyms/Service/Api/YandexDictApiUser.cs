...
namespace KamiJal.WordSynonyms.Service.Api
{
    public class YandexDictApiUser
    {
        ...
        {
            return await _httpClient.GetStringAsync($"lookup?key={_apiKey}&lang={lang}&text={_text}");
        }
        ...
        //JSON parsing
        private YandexDictApiObject Parse(string json)
        {
            return JsonConvert.DeserializeObject<YandexDictApiObject>(json);
        }
        ...
        private List<string> Synonyms(Translation translation)
        {
            return translation.syn?.Select(synonym => synonym.text).ToList();
        }
