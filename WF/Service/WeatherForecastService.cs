    ...
    public static class WeatherForecastService
    {
        ...
        public static async void Run([TimerTrigger("0 */5 * * * *")]TimerInfo myTimer, TraceWriter log)
        {
            ...
            try
            {
                ...
                weatherApiUser = new WeatherForecastApiUser(openWeatherApiKey, new List<string> {Almaty, Astana, Shymkent});
                ...
                await telegramBotApiUser.SendTextMessageAsync(CityWeatherForecast.CombineWeatherReport(cityWeatherForecasts));
            }
            catch (Exception ex)
            {
